...
pub async fn register_device_prod_impl<
    F: FcmService,
    M: UserMetadataStore,
    P: UserPrincipal,
...
pub async fn register_device_staging_impl<
    F: FcmService,
    M: UserMetadataStore,
    P: UserPrincipal,
    Req: RegisterDeviceRequest,
>(
    ...
    // Register device with FCM
    let (notification_key_from_firebase, is_create) = match &user_metadata.staging_notification_key
    {
        Some(existing_key) => {
            // Try to add to existing group
            match add_device_to_existing_group(
...
pub async fn unregister_device(
    ...
    if req.environment == "production" {
        ...
        unregister_device_staging_impl(
            &state.firebase_staging,
            &state.dragonfly_redis,
            user_principal,
            Json(req),
...
pub async fn unregister_device_prod_impl<
    F: FcmService,
    M: UserMetadataStore,
    P: UserPrincipal,
...
pub async fn unregister_device_staging_impl<
    F: FcmService,
    M: UserMetadataStore,
    P: UserPrincipal,
    Req: UnregisterDeviceRequest,
>(
    ...
    // Remove from Redis metadata
    if let Some(nk_meta) = user_metadata.staging_notification_key.as_mut() {
        nk_meta
            .registration_tokens
            .retain(|token| token.token != registration_token_obj.token);
